...
import WriteReview from "../component/writereview/writereview.jsx";
import ChooseReview from "../component/choosereview/choosereview.jsx";
import Settings from "../component/settings/settings.jsx";
import VievReview from "../component/viewreview/viewreview.jsx";
...
function MapToilet({ center }) {
  ...
  // Проходимось по кожному об'єкту у масиві
  markers.forEach((obj) => {
    // Перевіряємо, чи об'єкт coordinates вже має структуру lat та lng
    if (
      typeof obj.coordinates === "object" &&
      "lat" in obj.coordinates &&
      "lng" in obj.coordinates
    ) {
      // Якщо так, пропускаємо обробку цього об'єкта
      return;
    }
    // Розбиваємо рядок координат на lat та lng
    let [lat, lng] = obj.coordinates.split(", ");
    // Модифікуємо об'єкт, змінюючи формат координат
    obj.coordinates = { lat: parseFloat(lat), lng: parseFloat(lng) };
  });

  useEffect(() => {
    if (navigator.geolocation) {
      // Оновлюємо місцезнаходження користувача при завантаженні компонента
      updateUserLocation();
      // Встановлюємо інтервал для оновлення місцезнаходження кожні 5 секунд
      const intervalId = setInterval(updateUserLocation, 5000);
      // Прибираємо інтервал після розмонтовування компонента
